...
class _SessionHistoryState extends State<SessionHistory> {
  ...
  void _exportData(List<Session> sessions) {
    ...
    csv.writeln(
      'Session ID,Patient ID,Date,Start Time,Duration (mins),Status,Peak HR',
    );

    for (var s in sessions) {
      csv.writeln(
        '${s.id},${s.patientId},${DateFormat('yyyy-MM-dd').format(s.startTime)},${DateFormat('HH:mm').format(s.startTime)},${s.duration.inMinutes},${s.status},${s.peakHeartRate}',
      );
    }

    final blob = web.Blob([csv.toString().toJS].toJS);
    final url = web.URL.createObjectURL(blob);
    final anchor = web.document.createElement('a') as web.HTMLAnchorElement;
    anchor.href = url;
    anchor.download =
        "session_history_${DateFormat('yyyyMMdd_HHmm').format(DateTime.now())}.csv";
    anchor.click();
                ...
                child: StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (_searchController.text.isNotEmpty) {
                      ...
                      sessions = sessions
                          .where(
                            (s) => s.patientId.toLowerCase().contains(query),
                          )
                          .toList();
                    }
